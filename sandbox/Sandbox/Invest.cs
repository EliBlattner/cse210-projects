...
{
    public double _gainsOrLosses;
    public double investmentAccount;
    ...
    public void ExpandPortfolio(double _savingsAccount, double _checkingAccount)
    {
        ...
        if (askInvest == "y")
        {
            ...
            _gainsOrLosses = investmentAccount - (investmentAccount * percentRate);
            Thread.Sleep(1000);
            Console.WriteLine($"${_gainsOrLosses} was removed from your account and moved into the investment folder.");
            Console.WriteLine("...");
            Thread.Sleep(1000);
            Console.WriteLine($"After a year, your ${_gainsOrLosses} completely depreciated...That's too bad.");
            Thread.Sleep(1000);
        }
        else
        {
    ...
    public override void ClosingMessage(double _savingsAccount, double _checkingAccount)
    {
        ...
        Console.WriteLine("--- Thank you for Ivesting withaw Matrix Bank ---");
        Console.WriteLine($"\nYour current account balances are: \n\n   1. Savings - ${_savingsAccount}\n   2. Checking - ${_checkingAccount}");
        Thread.Sleep(1000);
    }
}